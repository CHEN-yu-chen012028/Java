        ...
        while (n < 4 || n > 10) {
            ...
            for (int i=0 ; i<n ; i++){
                if (i == wolfIndex){
                    ...
                    players[i] = new Player(i+1,role  = "Alive");
                }
            }
            System .out .prinln();
            System .out .print("Role assingnment start.");
            System.out .print ("Each player take turn to role ");
            for (int i=0;i<n;i++){
                System.out .println();
                System.out .println("Player",(i+1),"Please enter");
                sc.nextLine();
                System .out .println("Tour Role  : "+players[i].getRole());
            ...
            while(!gameOver ){
                ...
                System.out .println();
                System.out .println("Nightfalls.Werewolf wakes up.");
                int aliveWerewolf =findAliveWolf(players);
                if(aliveWerewolf!=-1){
                    ...
                    int target = -1;
                    while(true){
                        System .out .println("choose a players to kill");
                        if(sc.hasNext()){
                            targetId = sc.nextInt();
                            System.out .println();
                            if(is ValidTarget(targetId ,players[aliveWerewolf].getId())){
                                target = targetId;
                                break;
                            }else{
                    ...
                    System.out .println("night results"+targetId +"has been killed.");
                ...
                }else  if  (checkKillvillagerwin(players)){
                    System.out.println();
                    System.out .println("Wereeolf win!");
                    gameOver = true;
                }
                if(gameOver){
                    break;
               ...
               public static void printAlivePlayers(Player[] players){
                for(int i = 0 ; i < players ;i++){
                    if(players[i].isAlive()&& player[i].getRole().equals("Werewolf")){
                        return i;
                    }
                }
                return -1;
